...
/*弹出页面层*/
function layerPage(title,htmlContent,oparent,options){
    var opts={
        title:title,
...
var base_openYhxzPage = function(idInput, mcInput, params,oparent){
    var url = sys_ctx+'/base/util/yhxz';
    var show='';
    url += '?show='+show + (typeof params === 'string' ? "&"+params : "");
    url+="&idInput="+idInput+"&mcInput="+mcInput;
    layer_show("用户选择",url,'450','500',oparent,null,{shadeClose:false,maxmin:false})
}
//图标选择
var base_openIconPage = function(idInput, nowValue,oparent){
    var url = sys_ctx+'/base/util/iconxz';
    url += '?value='+nowValue;
    url+="&idInput="+idInput;
    layer_show("图标选择",url,'','',oparent,null,{shadeClose:false,maxmin:false})
}

//资源分类选择
var base_openZyflxzPage = function(idInput, mcInput, params,oparent){
    var url = sys_ctx+'/cms/util/zyflxz';
    var show='';
    url += '?show='+show + (typeof params === 'string' ? "&"+params : "");
    url+="&idInput="+idInput+"&mcInput="+mcInput;
    layer_show("资源分类选择",url,'400','500',oparent,null,{shadeClose:false,maxmin:false})
}
...
function errorException(data){
    ...
    if(error!=null&&error!=undefined)
    {
        ...
        switch(code){
            case "SessionTimeout":
                alert("errorException:"+msg);
                top.window.location=sys_ctx+"/login";
                break;
            case "500":
            case 500:
               // alert(msg);
                layerMsg_cryIcon("服务器内部错误！");
                break;
        }
    }
