...
<%@ attribute name="layui" type="java.lang.Boolean" required="false" description="是否layui样式"%>
...
var province="",city1="",city2="",homeTown="";
if(${layui}==true){
	$("#${id}").ProvinceCityLayui();
	layui.use(['form'], function() {
		...
		form.on('select(hui-city1)', function(data) {
			city1=data.value;alert(city1);
		});
		form.on('select(hui-city2)', function(data) {
			city2=data.value;	alert(city2);
		});
		homeTown=province+" "+city1+" "+city2;
