  ...
  <form action="" id="form_show" method="post" class="form form-horizontal" id="form-admin-role-add">
    ...
    <div class="row cl">
      ...
      <div class="formControls col-xs-8 col-sm-5">
        <input type="text" class="input-text" value="${role.name}" placeholder="" id="name" name="name" datatype="*4-16" nullmsg="请输入角色名称">
      </div>
    ...
    <div class="row cl">
      ...
      <div class="formControls col-xs-8 col-sm-5">
        <textarea rows="3" style="height: 70px;" class="input-text"  placeholder="" id="description" name="description">${role.description}</textarea>
      </div>
    </div>
    <div class="row cl">
      ...
      <div class="formControls col-xs-8 col-sm-9">
       ...
       <c:forEach var="per" varStatus="i" items="${permissions}">
          <dl class="permission-list">
            <dt>
              <label class="mk">
                <input type="checkbox" <c:if test="${per.checked=='1'}">checked="checked" </c:if> value="${per.id}" name="user-Character-${i.count}" id="user-Character-${i.count}">
                  ${per.name}</label>
            </dt>
            <dd>
              <c:forEach var="per2" varStatus="j" items="${per.children}">
                <dl class="cl permission-list2">
                  <dt>
                    <label class="ym">
                      <input type="checkbox" <c:if test="${per2.checked=='1'}">checked="checked" </c:if> value="${per2.id}" name="user-Character-${i.count}-${j.count}" id="user-Character-${i.count}-${j.count}">
                        ${per2.name}</label>
                  </dt>
                  ...
                  <dd>
                    <c:forEach var="per3" varStatus="k" items="${per2.children}">
                      <label class="ff">
                        <input type="checkbox" <c:if test="${per3.checked=='1'}">checked="checked" </c:if> value="${per3.id}" name="user-Character-${i.count}-${j.count}-${k.count}" id="user-Character-${i.count}-${j.count}-${k.count}">
                          ${per3.name}</label>
                    </c:forEach>
                  </dd>
