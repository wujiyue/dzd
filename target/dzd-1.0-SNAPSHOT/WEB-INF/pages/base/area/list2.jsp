...
<head>
    ...
    function loadTree(){
        $('#trees').tree( {
            ...
            onClick:function(node){
                ...
                if(id!='0'&&id!=0){
                    tmp_areatype=node.attributes.areatype;
                }else{
                    tmp_areatype="ROOT";
                }
                if(tmp_areatype=="ROOT"||tmp_areatype=="PROVINCE"||tmp_areatype=="CITY"){
    ...
    function add(){
        var params="?parentid="+tmp_parentid+"&parentname="+tmp_parentname+"&areatype="+tmp_areatype
        layer_show("新增区域",sys_ctx+"/base/area/add"+params,'','','',function(){sys_table_list();refreshTree();});
    }
    //根据表单查询
    var query = function(formid){
     ...
     var sys_table_list = function(){
          var columns = [[
			...
			{field:'parentid',title:'父级编号',align:'center',width:table_list_td_width},
			{field:'parentids',title:'所有父级编号',align:'center',width:table_list_td_width},
			{field:'code',title:'区域编码',align:'center',width:table_list_td_width},
			{field:'name',title:'区域名称',align:'center',width:table_list_td_width},
			{field:'sort',title:'排序',align:'center',width:table_list_td_width},
			{field:'areatype',title:'区域类型',align:'center',width:table_list_td_width}
           	]];
          ...
          table_list_dataGrid.url = sys_ctx+'/base/area/json/find';
     ...
     var onClickRow=function(rowIndex, rowData){
          ...
          if(sys_page_config.showid != ''){
            layer_show("编辑区域",sys_ctx+"/base/area/edit?id="+rowData[sys_page_config.table_field],'','','',function(){sys_table_list();refreshTree();});
          }
        }
     function save(){
     ...
     function list_del(tableid){
           ...
           if(selecRow.length > 0){
             layer.confirm("确定这样做吗？", function(){
               ...
               $.ajax({
                 type:"post",
                 url:'${ctx}/base/area/json/removes/'+ids,
                 data:null,
    ...
    var saveSort = function(){
        ...
        $.ajax({
            type:"post",
            url:'${ctx}/base/area/json/saveSort',
            data:{sort:sort},
            success:function(data,textStatus){
                broAjaxReturnMsg(data);
                query("form_query");
...
<body>
<!--面包屑导航条-->
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 系统管理 <span class="c-gray en">&gt;</span> 区域管理<a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
