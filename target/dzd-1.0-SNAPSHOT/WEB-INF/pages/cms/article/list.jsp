...
<script type="text/javascript">
    ...
    function add(){
        layer_show_full("新增文章",sys_ctx+"/cms/article/add",'','','',function(){sys_table_list();});
    }
    //根据表单查询
    var query = function(formid){
     ...
     var sys_table_list = function(){
          var columns = [[
			...
			{field:'userid',title:'作者',align:'center',width:table_list_td_width},
			{field:'datascope',title:'数据范围',align:'center',width:table_list_td_width},
			{field:'title',title:'文章标题',align:'center',width:table_list_td_width},
			{field:'keywords',title:'关键词',align:'center',width:table_list_td_width},
			{field:'description',title:'摘要',align:'center',width:table_list_td_width},
			{field:'content',hidden:true,title:'文章内容',align:'center',width:table_list_td_width},
			{field:'articletypes',title:'文章类别',align:'center',width:table_list_td_width},
			{field:'channelids',title:'频道栏目ID',align:'center',width:table_list_td_width},
			{field:'source',title:'来源',align:'center',width:table_list_td_width},
			{field:'tags',title:'标签',align:'center',width:table_list_td_width}
           	]];
          
          table_list_dataGrid.queryParams = table_list_query_form;
          table_list_dataGrid.pagination = true;
          table_list_dataGrid.url = sys_ctx+'/cms/article/json/find';
     ...
     var onClickRow=function(rowIndex, rowData){
          ...
          if(sys_page_config.showid != ''){
            layer_show_full("编辑文章",sys_ctx+"/cms/article/edit?id="+rowData[sys_page_config.table_field],'','','',function(){sys_table_list();});
          }
        }
     function save(){
     ...
     function list_del(tableid){
           ...
           if(selecRow.length > 0){
             layer.confirm("确定这样做吗？", function(){
               ...
               $.ajax({
                 type:"post",
                 url:'${ctx}/cms/article/json/removes/'+ids,
                 data:null,
     ...
     $(function(){
           ...
           table_list_td_width = parseFloat(($("#table_list").parent().parent().parent().width()-(33+(6)-1))/10).toFixed(2);
...
<body>
<!--面包屑导航条-->
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 内容管理 <span class="c-gray en">&gt;</span> 资讯管理<a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
