...
<!--面包屑导航条-->
<nav class="breadcrumb"><i class="fa fa-home"></i> 首页 <span class="c-gray en">&gt;</span> cms <span class="c-gray en">&gt;</span>便签管理管理</nav>
...
<script type="text/javascript" >
    ...
    function sys_table_list(){
        ...
        table_list_Params.url='${ctx}/cms/labelMsg/json/find'+queryStr;
        $('#tableList').bootstrapTable(table_list_Params);
    }

    var onClickRow=function(row,tr){
        layer_show("编辑便签管理",sys_ctx+"/cms/labelMsg/edit?id="+row.id,'','','',function(){sys_table_list();});
    }
    function addNew(){
       layer_show("新增便签管理",sys_ctx+"/cms/labelMsg/add",'','','',function(){sys_table_list();});
    }
    ...
    function list_del(tableid){
        ...
        if(selecRow.length > 0){
            layer.confirm("确定这样做吗？", function(){
                ...
                $.ajax({
                    type:"post",
                    url:'${ctx}/cms/labelMsg/json/deletes/'+ids,
                    data:null,
