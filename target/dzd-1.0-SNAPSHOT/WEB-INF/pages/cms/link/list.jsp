...
<script type="text/javascript">
    ...
    function add(){
        layer_show("新增链接",sys_ctx+"/cms/link/add",'','','',function(){sys_table_list();});
    }
    //根据表单查询
    var query = function(formid){
     ...
     var sys_table_list = function(){
          var columns = [[
			...
			{field:'mkdm',title:'所属模块',align:'center',width:table_list_td_width},
			{field:'userid',title:'用户id',align:'center',width:table_list_td_width},
			...
			{field:'linktypes',title:'类型',align:'center',width:table_list_td_width},
			{field:'tags',title:'标签',align:'center',width:table_list_td_width},
			{field:'href',title:'跳转链接',align:'center',width:table_list_td_width},
			{field:'target',title:'打开目标',align:'center',width:table_list_td_width},
			{field:'img',title:'链接图片1',align:'center',width:table_list_td_width},
			{field:'img2',title:'链接图片2',align:'center',width:table_list_td_width},
          ...
          table_list_dataGrid.url = sys_ctx+'/cms/link/json/find';
     ...
     var onClickRow=function(rowIndex, rowData){
          ...
          if(sys_page_config.showid != ''){
            layer_show("编辑链接",sys_ctx+"/cms/link/edit?id="+rowData[sys_page_config.table_field],'','','',function(){sys_table_list();});
          }
        }
     function save(){
     ...
     function list_del(tableid){
           ...
           if(selecRow.length > 0){
             layer.confirm("确定这样做吗？", function(){
               ...
               $.ajax({
                 type:"post",
                 url:'${ctx}/cms/link/json/removes/'+ids,
                 data:null,
...
<body>
<!--面包屑导航条-->
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> cms <span class="c-gray en">&gt;</span> 链接管理<a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
