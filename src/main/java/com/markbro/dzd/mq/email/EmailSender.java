package com.markbro.dzd.mq.email;


import com.markbro.asoiaf.core.config.Resources;
...
public final class EmailSender {
	...
	public void setSmtpHost(String hostName) {
		...
		//	hostName = PropertiesUtil.getString("email.smtp.host");
			hostName = Global.getConfig("email.smtp.host");
		}
	...
	private void setNeedAuth() {
		...
		if (userkey == null || userkey.trim().equals("")) {
			...
			userkey = Global.getConfig("email.smtp.host");
		}
		if (userkey == null || userkey.trim().equals("")) {
	...
	public void setNamePass(String name, String pass, String key) {
		if (name == null || name.trim().equals("")) {
			...
			name=Global.getConfig("email.user.name");
		}
		if (pass == null || pass.trim().equals("")) {
			//pass = PropertiesUtil.getString("email.user.password");
			pass =Global.getConfig("email.user.password");
		}
		username = name;
		password = pass;
		userkey = key;
	...
	public boolean setFrom(String from) {
		if (from == null || from.trim().equals("")) {
			...
			from=Global.getConfig("email.send.from");
		}
		try {
			String[] f = from.split(",");
			if (f.length > 1) {
