package com.markbro.dzd.cms.timeline.web;
...
import com.markbro.dzd.cms.timeline.service.TimelineService;
 ...
 */
@Controller
@RequestMapping("/cms/timeline")
public class TimelineController extends com.markbro.asoiaf.core.web.BaseController{
    @Autowired
    protected TimelineService timelineService;
    @RequestMapping(value={"","/"})
    public String index(){
        return "/cms/timeline/list";
    }
    /**
     * 跳转到新增页面
     */
    ...
    public String toEdit(Model model){
        ...
        Map<String,Object> timeline=timelineService.getMap(id);
        model.addAttribute("timeline",timeline);
        return "/cms/timeline/edit";
    }
    /**
     * 删除数据并重定向到列表页面
     */
     ...
     public String delete (@PathVariable java.lang.Integer id, RedirectAttributes redirectAttributes) {
            ...
            return "redirect:/cms/timeline/list";
        }
    /**
     * 跳转到列表页面
     */
    ...
    public String list(PageParam pageParam,Model model){
        ...
        timelines=timelineService.find(getPageBounds(pageParam),getMap(request));
    ...
    */
    @RequestMapping(value="/save",method = RequestMethod.POST)
        public String save(Timeline timeline,
                           RedirectAttributes redirectAttributes,Model model){
            if(timeline.getId()==null){//新增保存
                try {
                    ...
                    return "redirect:/cms/timeline/add";
                }
            }else{//编辑保存
                try {
                    timeline.setAvailable(1);//如果不设置1，默认0会有问题的
                    ...
                    redirectAttributes.addFlashAttribute("timeline", timeline);
                    return "redirect:/cms/timeline/edit";
                }
            }
            return "redirect:/cms/timeline/list";
        }
	...
	*/
	...
	public Object findDeleted() {
		...
		map.put("deleted",1);
		return timelineService.findByMap(getPageBounds(),map);
	}
	...
	public void findBack(@PathVariable java.lang.Integer[] ids) {
		...
		map.put("deleted",0);
		map.put("ids",ids);
		timelineService.updateByMapBatch(map);
	}
    /**
	...
	*/
    ...
    public Object find() {
        resultMap=getPageMap(timelineService.find(getPageBounds(),getMap(request)));
        return resultMap;
    }
    /**
     * 不分页查询数据
    ...
    public Object findAll() {
        ...
        resultMap=getPageMap(timelineService.find(pageBounds,map));
        return resultMap;
    }
    @ResponseBody
    @RequestMapping(value="/json/add",method = RequestMethod.POST)
    @ActionLog(description="新增时光轴")
    public void add(Timeline m) {
        
        timelineService.add(m);
    }
    @ResponseBody
    @RequestMapping(value="/json/update",method = RequestMethod.POST)
    public void update(Timeline m) {
        timelineService.update(m);
    }
    @ResponseBody
    @RequestMapping(value="/json/save",method = RequestMethod.POST)
    @ActionLog(description="保存时光轴")
    public Object save() {

        return  timelineService.save(getMap(request));
    }
    /**
	* 逻辑删除的数据（deleted=1）
	*/
	...
	try{
		...
		map.put("deleted",1);
		map.put("id",id);
		timelineService.updateByMap(map);
	...
	return msg;
     ...
     public Object deletes(@PathVariable java.lang.Integer[] ids) {//前端传送一个用逗号隔开的id字符串，后端用数组接收，springMVC就可以完成自动转换成数组
        ...
        try{
             ...
             timelineService.deleteBatch(ids);
             msg.setType(Msg.MsgType.success);
             msg.setContent("删除成功！");
        }catch (Exception e){
             msg.setType(Msg.MsgType.error);
