package com.markbro.dzd.common.excel;
...
public class ExcelUtils {
	...
	public static TableHeaderMetaData createTableHeader(String[] titls, int[] columnType){
		
		if(titls.length != columnType.length){
			//System.out.println("请输入正确的表头与列类型;两个参数的长度必需相同;");
			return null;
		}
		TableHeaderMetaData headMeta = new TableHeaderMetaData();
		int i=0;
	...
	public static String dumpFont(HSSFFont font) {
		...
		sb.append(font.getItalic()).append(",").append(font.getStrikeout())
				.append(",").append(font.getBoldweight())
				.append(",").append(font.getCharSet())
				.append(",").append(font.getColor())
				.append(",").append(font.getFontHeight())
				.append(",").append(font.getFontName())
				.append(",").append(font.getTypeOffset())
				.append(",").append(font.getUnderline());
		return sb.toString();
	}
