...
<script type="text/javascript">
    ...
    function add(){
        layer_show("新增产品",sys_ctx+"/cms/product/add",'','','',function(){sys_table_list();});
    }
    //根据表单查询
    var query = function(formid){
     ...
     var sys_table_list = function(){
          var columns = [[
			...
			{field:'cpsm',title:'产品说明',align:'center',width:table_list_td_width},
			{field:'zcfs',title:'注册方式',align:'center',width:table_list_td_width},
			{field:'zcmy',title:'注册密钥',align:'center',width:table_list_td_width}
           	]];
          
          table_list_dataGrid.queryParams = table_list_query_form;
          table_list_dataGrid.pagination = true;
          table_list_dataGrid.url = sys_ctx+'/cms/product/json/find';
     ...
     var onClickRow=function(rowIndex, rowData){
          ...
          if(sys_page_config.showid != ''){
            layer_show("编辑产品",sys_ctx+"/cms/product/edit?id="+rowData[sys_page_config.table_field],'','','',function(){sys_table_list();});
          }
        }
     function save(){
     ...
     function list_del(tableid){
           ...
           if(selecRow.length > 0){
             layer.confirm("确定这样做吗？", function(){
               ...
               $.ajax({
                 type:"post",
                 url:'${ctx}/cms/product/json/removes/'+ids,
                 data:null,
...
<body>
<!--面包屑导航条-->
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> cms <span class="c-gray en">&gt;</span> 产品管理<a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
