...
<script type="text/javascript">
    var table_list_td_width = 80;
    //查询功能的标签说明
    var table_list_query_form = {
    };
    function add(){
        layer_show_full("新增图片",sys_ctx+"/cms/image/add",'','','',function(){sys_table_list();});
    }
    //根据表单查询
    var query = function(formid){
     ...
     var sys_table_list = function(){
          var columns = [[
			...
			{field:'yhid',title:'用户id',align:'center',width:80},
			{field:'datascope',title:'数据访问类型',align:'center',width:80},
			{field:'name',title:'图片名称',align:'center',width:table_list_td_width},
			{field:'url',title:'地址',align:'center',width:table_list_td_width},
			{field:'url_small',title:'小图地址',hidden:true,align:'center',width:table_list_td_width},
			{field:'size',title:'大小',align:'center',width:80,formatter:function(v){
                v=Number(v);
                if(v>0){
                    return (v/1024).toFixed(2)+"KB";
                }
            }},
			{field:'suffixes',title:'后缀',align:'center',width:80},
			{field:'keywords',title:'关键词',align:'center',width:table_list_td_width},
			{field:'resourcetypeid',title:'资源类型',align:'center',width:table_list_td_width}
           	]];
          
          table_list_dataGrid.queryParams = table_list_query_form;
          table_list_dataGrid.pagination = true;
          table_list_dataGrid.url = sys_ctx+'/cms/image/json/find';
     ...
     var onClickRow=function(rowIndex, rowData){
          ...
          if(sys_page_config.showid != ''){
              layer_show_full("编辑图片",sys_ctx+"/cms/image/edit?id="+rowData[sys_page_config.table_field],'','','',function(){sys_table_list();});
          }
        }
     function save(){
     ...
     function list_del(tableid){
           ...
           if(selecRow.length > 0){
             layer.confirm("确定这样做吗？", function(){
               ...
               $.ajax({
                 type:"post",
                 url:'${ctx}/cms/image/json/removes/'+ids,
                 data:null,
...
<body>
<!--面包屑导航条-->
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 内容管理 <span class="c-gray en">&gt;</span> 图片管理<a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
