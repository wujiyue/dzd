...
<script type="text/javascript">
    ...
    function add(){
        layer_show("新增资源",sys_ctx+"/cms/resource/add",'','','',function(){sys_table_list();});
    }
    //根据表单查询
    var query = function(formid){
     ...
     var sys_table_list = function(){
          var columns = [[
			...
			{field:'resourcetypeid',title:'分类Id',align:'center',width:table_list_td_width},
			{field:'name',title:'资源名称',align:'center',width:table_list_td_width},
			{field:'keywords',title:'关键词',align:'center',width:table_list_td_width},
			{field:'from',title:'来源',align:'center',width:table_list_td_width},
			{field:'url',title:'链接',align:'center',width:table_list_td_width},
			{field:'urlsmall',hidden:true,title:'小图',align:'center',width:table_list_td_width},
			{field:'locaurl',title:'本地路径',align:'center',width:table_list_td_width},
			{field:'suffix',hidden:true,title:'后缀',align:'center',width:table_list_td_width},
			{field:'targetid',title:'记录的id',align:'center',width:table_list_td_width},
			{field:'resourcestate',title:'资源状态',align:'center',width:table_list_td_width}
           	]];
          table_list_dataGrid.queryParams = table_list_query_form;
          table_list_dataGrid.pagination = true;
          table_list_dataGrid.url = sys_ctx+'/cms/resource/json/find';
     ...
     var onClickRow=function(rowIndex, rowData){
          ...
          if(sys_page_config.showid != ''){
            layer_show("编辑资源",sys_ctx+"/cms/resource/edit?id="+rowData[sys_page_config.table_field],'','','',function(){sys_table_list();});
          }
        }
     function save(){
     ...
     function list_del(tableid){
           ...
           if(selecRow.length > 0){
             layer.confirm("确定这样做吗？", function(){
               ...
               $.ajax({
                 type:"post",
                 url:'${ctx}/cms/resource/json/removes/'+ids,
                 data:null,
...
<body>
<!--面包屑导航条-->
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 内容管理 <span class="c-gray en">&gt;</span> 资源管理<a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
