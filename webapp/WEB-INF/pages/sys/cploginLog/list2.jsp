...
<script type="text/javascript">
    ...
    function add(){
        layer_show("新增登录日志",sys_ctx+"/sys/cploginLog/add",'','','',function(){sys_table_list();});
    }
    //根据表单查询
    var query = function(formid){
     ...
     var sys_table_list = function(){
          var columns = [[
			...
			{field:'yhid',title:'用户ID',align:'center',width:table_list_td_width},
			{field:'yhmc',title:'用户名称',align:'center',width:table_list_td_width},
			{field:'fjh',title:'',align:'center',width:table_list_td_width},
			{field:'sf',title:'登录的省份',align:'center',width:table_list_td_width},
			{field:'cpdm',title:'产品代码',align:'center',width:table_list_td_width},
			{field:'cpmc',title:'产品名称',align:'center',width:table_list_td_width},
			{field:'version',title:'版本号',align:'center',width:table_list_td_width},
			{field:'logintime',title:'登录时间',align:'center',width:table_list_td_width},
			{field:'ip',title:'登录IP',align:'center',width:table_list_td_width}
           	]];
          
          table_list_dataGrid.queryParams = table_list_query_form;
          table_list_dataGrid.pagination = true;
          table_list_dataGrid.url = sys_ctx+'/sys/cploginLog/json/find';
     ...
     var onClickRow=function(rowIndex, rowData){
          ...
          if(sys_page_config.showid != ''){
            layer_show("编辑登录日志",sys_ctx+"/sys/cploginLog/edit?id="+rowData[sys_page_config.table_field],'','','',function(){sys_table_list();});
          }
        }
     function save(){
     ...
     function list_del(tableid){
           ...
           if(selecRow.length > 0){
             layer.confirm("确定这样做吗？", function(){
               ...
               $.ajax({
                 type:"post",
                 url:'${ctx}/sys/cploginLog/json/removes/'+ids,
                 data:null,
...
<body>
<!--面包屑导航条-->
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> sys <span class="c-gray en">&gt;</span> 登录日志管理<a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
