...
function loadinfo(){
		...
		sys_ajaxPost("/wx/default.do?method=queryFpsmList&username="+username+"&fpdm="+fpdm+"&fjh="+fjh+"&kprq_q="+kprq_q+"&kprq_z="+kprq_z+"&rnd="+new Date().getTime(), null, function (json) {
			...
			if(json.result==true||json.result=='true'){
				...
				if(Number(total)>0){
					...
					var qyxxlist="";
					qyxxlist+='<span class="weui_grid grid_title">发票代码</span>';
